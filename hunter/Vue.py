    ...
    for r in range(nbCase):
        ...
        for c in range(nbCase):
            ...
            if sma.environnement.getInstance().espace[r][c]!=None:
                if isinstance(sma.environnement.getInstance().espace[r][c], Brick):
                    Affich[(r, c)]= Can.create_rectangle(x-((taille_canvas//2)//nbCase),y-((taille_canvas//2)//nbCase),x+((taille_canvas//2)//nbCase),y+((taille_canvas//2)//nbCase),fill=sma.environnement.getInstance().espace[r][c].color)
                else:
...
Label (frame1, text="Veuillez entrer la vitesse du hunter (max:1)").grid(row=0, column=2)
Label(frame1,text= "Veuillez entrer le pourcentage d'obsatcles").grid(row=1,column=0)
Label (frame1, text="Veuillez entrer la vitesse de l'avatar (max:1)").grid(row=1, column=2)

Label(frame1,text= "Veuillez entrer la taille de la grille").grid(row=2,column=0)
Label (frame1, text="Veuillez entrer la duree d'invincibilite de l'avatar (tours)").grid(row=2, column=2)
Label(frame1,text= "Veuillez entrer le nombre de defenders ").grid(row=3,column=0)
Label(frame1,text= "Veuillez entrer la durée de vie des defenders (tours avant respawn)").grid(row=3,column=2)
Label(frame1,text= "Veuillez entrer le nombre de tours (0 = infini)").grid(row=4,column=0)
